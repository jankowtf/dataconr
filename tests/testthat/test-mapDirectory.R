...
test_that("mapDirectory::default", {
  expect_is(
    res <- withCorrectWorkingDir(
      mapDirectory(dir = "data/persistent/mapDirectory_1")
    ),
    "list"
  )
...
test_that2("mapDirectory::include_root", {
  expect_is(res <- mapDirectory(dir = "data/persistent/mapDirectory_1",
...
test_that("mapDirectory_2::include_files", {
  expect_identical(
    withCorrectWorkingDir(
      res <- mapDirectory(dir = "data/persistent/mapDirectory_2",
        include_files = FALSE)
    ),
    list(
      data = list(
        persistent = list(
          mapDirectory_2 = list(R = list(scripts = list()))
        )
      )
    )
  )
